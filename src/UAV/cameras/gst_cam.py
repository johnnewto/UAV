...
try:
    from gstreamer import GstPipeline, GstVideoSource, GstVideoSave, GstJpegEnc, GstStreamUDP, Gst, CallbackHandler
...
class GSTCamera(CV2Camera):
    ...
    def __init__(self,
                 ...
                 run_rtsp_server=False,  # set as RTSP server
                 loglevel=LogLevels.INFO):  # log flag
        super().__init__(camera_dict, loglevel)
        ...
        self.run_rtsp_server = run_rtsp_server
        ...
        self.tracking_handler: CallbackHandler | None = None
    ...
    def _setup_video_stream(self, streamId=0):  # Stream ID (0 for all streams
        ...
        self._pipeline_stream_video: GstStreamUDP = GstStreamUDP(pipeline, on_callback=self.on_video_callback, loglevel=self._loglevel).startup()
        self._pipeline_stream_video.pipeline.set_name('gstreamer_udpsink')
        self.log.info(f'Video streaming pipeline "{self._pipeline_stream_video.pipeline.get_name()}" created on port {self._stream_dict["port"]}')
        time.sleep(0.1)
    ...
    def set_camera_mode(self, param):
        """ Set camera mode
        0	CAMERA_MODE_IMAGE
        1	CAMERA_MODE_VIDEO
        ...
        self.log.info(f"{self.cam_name = } {self.camera_mode = }")
        if param == 3:
            self.start_tracking()
        else:
            try:
                self.log.info(f'Track pipeline "{self._gst_track.pipeline.get_name()}" closing')
                self._start_tracking_thread_is_done = True
    ...
    def start_tracking(self ):
        ...
        self._gst_track.pipeline.set_name('gstreamer_track')
        self.log.info(f'Track pipeline "{self._gst_track.pipeline.get_name()}" created')
        time.sleep(0.1)  # wait for pipeline to start, then see if it is active (i.e not shuttdown)
        if not self._gst_track.is_active:
            raise Exception(f"Error in {self._gst_track.pipeline.get_name()}")
        # self._gst_track_thread = threading.Thread(target=self._start_tracking_thread)
        # self._gst_track_thread.start()

    # def _start_tracking_thread(self):
    #     # self._end_jpeg_capture_event.clear()
    #     self._start_tracking_thread_is_done = False
    #     while not self._start_tracking_thread_is_done: #  and not self._end_stream_event.is_set():
    #         buffer = self._gst_track.get_nowait()
    #         if not buffer:
    #             pass
    #             # self.log.warning("No buffer")
    #         else:
    #             self.log.info(f"Got buffer: {buffer}")
    ...
    def set_tracking_callback(self, callback):
        self._gst_track.callback  = callback
    ...
    def close(self):
        ...
        if self._pipeline_stream_video is not None:
            self._pipeline_stream_video.shutdown()
            self.log.info(f'!!!!!! Closed "{self._pipeline_stream_video.pipeline.get_name()}" ')
