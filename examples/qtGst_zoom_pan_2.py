...
from gstreamer import GstPipeline, Gst, GstContext, GstPipes, GstReceiveUDP
...
class VideoWidget(QMainWindow):
    def __init__(self):
        ...
        if cap:
            ...
            print(f"{self.video_width =}, {self.video_height =}")
        else:
            self.video_width = 640
            self.video_height = 480
            print(f"No Caps , seeting {self.video_width =}, {self.video_height =}")
    ...
    def init_gstreamer(self):
        ...
        #     # 'autovideosink',
        # ])
        self.pipeline = GstReceiveUDP(command=command, window_handle=self.window_handle, loglevel=10)
...
def run_sink(num_cams=2, udp_encoder='h264'):
    command = to_gst_string([
        'udpsrc port=5000 name = src ! application/x-rtp, media=video, clock-rate=90000, encoding-name=H264, payload=96',
        'queue',
        'rtph264depay ! avdec_h264',
    ...
    with GstReceiveUDP(command) as rcv_pipeline:
        while rcv_pipeline.is_active:
            time.sleep(0.1)
