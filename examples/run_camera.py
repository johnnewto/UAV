...
if __name__ == '__main__':
    ...
    cam_gst_1 = GSTCamera(camera_dict=read_camera_dict_from_toml(config_path / "test_camera_info.toml"), debug=False)
        ...
        with GstPipeline(SINK_PIPELINE, debug=False) as rcv_pipeline: # this will show the video on fpsdisplaysink
            with MAVCom(con1, source_system=111, debug=False) as client:
                with MAVCom(con2, source_system=222, debug=False) as server:
                    ...
                    cam.video_start_capture(frequency=1)
                    time.sleep(5)

                    cam.video_stop_capture()
                    # time.sleep(0.1)
                    ...
                    for i in range (2):
                        ...
                        time.sleep(1)
                    #
                    # # while cam_gst_1.pipeline._pipeline._main_loop.is_running():
                    # #     time.sleep(0.1)
                    #
                    cam.image_start_capture(interval=0.1, count=5)
                    # time.sleep(4)
                    # time.sleep(0.1)
